...
import { SOAL_PENGETAHUAN, useKuesionerHandler } from './kuesionerPengetahuan';

export default function KuesionerPengetahuan() {
    ...
    if (result) {
        const isBaik = result.kategori === "Baik";
        return (
            <div className="max-w-5xl mx-auto p-4 md:p-8 min-h-screen">
                ...
                <div className="mb-6 md:mb-10 text-center md:text-left mt-4 md:mt-0">
                    <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">
                        Hasil <span className="text-[#D81B60]">Kuesioner</span>
                    </h1>
                    <p className="mt-2 text-gray-500 text-sm md:text-base">
                        Evaluasi pemahaman Ibu tentang gizi seimbang dan kelancaran ASI.
                    </p>
                </div>
                ...
                <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-xl w-full max-w-3xl mx-auto text-center border border-pink-50">
                    ...
                    <div className="bg-pink-50/50 rounded-3xl p-8 mb-8 border border-pink-100">
                        ...
                        <span className={`inline-block px-6 py-2.5 rounded-full text-sm font-bold tracking-widest uppercase ${
                            isBaik ? "bg-emerald-100 text-emerald-700" : result.kategori === "Cukup" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"
                        }`}>
              Kategori: {result.kategori}
                    ...
                    <div className="text-left text-base text-gray-600 bg-gray-50 p-6 md:p-8 rounded-2xl border border-gray-100 mb-10">
                        ...
                        <p className="leading-relaxed">
                            {result.kategori === "Baik" && "Ibu memiliki pengetahuan yang sangat baik tentang gizi seimbang dan ASI eksklusif. Pertahankan kebiasaan yang sudah baik ini ya, Bun!"}
                            {result.kategori === "Cukup" && "Pengetahuan Ibu sudah cukup baik, namun masih bisa ditingkatkan. Terus cari informasi lebih lanjut mengenai gizi seimbang selama menyusui."}
                            {result.kategori === "Kurang" && "Ibu memerlukan pemahaman lebih lanjut mengenai gizi dan ASI. Disarankan untuk membaca materi edukasi di aplikasi atau berkonsultasi dengan ahli gizi."}
                        </p>
                    </div>
    ...
    const currentSoal = SOAL_PENGETAHUAN[currentIndex];
    const isAnswered = answers[currentIndex] !== undefined;

    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 min-h-screen flex flex-col">

            {/* Header Selaras (Seperti Gambar 2) */}
            <div className="mb-6 md:mb-8 text-center md:text-left mt-4 md:mt-0">
                <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">
                    Pengetahuan <span className="text-[#D81B60]">Gizi & ASI</span>
                </h1>
                <p className="mt-2 text-gray-500 text-sm md:text-base max-w-2xl">
                    Evaluasi pemahaman Ibu tentang gizi seimbang untuk memastikan produksi dan pengeluaran ASI berjalan optimal.
                </p>
            </div>

            {/* Main Card Split Layout (Seperti Gambar 1) */}
            <div className="bg-white rounded-[2rem] shadow-xl border border-gray-100 flex flex-col md:flex-row overflow-hidden flex-1 md:min-h-[600px]">
                ...
                <div className="w-full md:w-1/3 bg-gray-50/50 p-6 md:p-8 md:border-r border-gray-100 hidden md:block overflow-y-auto">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <div className="w-12 h-12 rounded-full border-4 border-pink-100 flex items-center justify-center">
                            <span className="text-xs font-bold text-[#D81B60]">{currentIndex + 1}/{SOAL_PENGETAHUAN.length}</span>
                        </div>
                    </div>

                    {/* Vertical Stepper */}
                    <div className="space-y-0 relative">
                        {SOAL_PENGETAHUAN.map((soal, index) => {
                            const isActive = index === currentIndex;
                            const hasAnswered = answers[index] !== undefined;
                            const isLast = index === SOAL_PENGETAHUAN.length - 1;

                            return (
                                <div key={index} className="relative flex items-start group">
                                    {/* Garis penghubung (hilangkan di item terakhir) */}
                                    {!isLast && (
                ...
                <div className="w-full md:w-2/3 p-6 md:p-12 flex flex-col bg-white">
                    ...
                    {/* Progress Mobile (Hanya muncul di HP) */}
              ...
              {currentIndex + 1} / {SOAL_PENGETAHUAN.length}
            </span>
                    </div>
                    ...
                    <div className="flex-1">
                        ...
                        {/* Pilihan Jawaban dengan gaya Radio modern */}
                                ...
                                <button
                                    ...
                                    className={`w-full text-left p-5 rounded-2xl border transition-all duration-200 flex items-center gap-4 group ${
                                        answers[currentIndex] === key
                                            ? "border-[#D81B60] bg-pink-50/30"
                                            : "border-gray-200 hover:border-pink-300 hover:bg-gray-50"
                                    }`}
                                >
                                    {/* Radio Icon Circle */}
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                        answers[currentIndex] === key ? "border-[#D81B60]" : "border-gray-300 group-hover:border-pink-300"
                                    }`}>
                                        {answers[currentIndex] === key && <div className="w-3 h-3 rounded-full bg-[#D81B60]"></div>}
                                    </div>

                                    <span className={`text-base md:text-lg ${answers[currentIndex] === key ? "text-gray-900 font-medium" : "text-gray-600"}`}>
                    {value}
                  </span>
                                </button>
                            ))}
                    ...
                    <div className="mt-10 pt-8 border-t border-gray-100 flex items-center justify-between gap-4">
                        ...
                        >
                            ...
                            ) : currentIndex === SOAL_PENGETAHUAN.length - 1 ? (
                                "Kirim Jawaban"
                            ) : (
