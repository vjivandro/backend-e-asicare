...
import { getPerilakuMenyusui } from "../monitoringService";
...
export default function Menyusui() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getPerilakuMenyusui();
      setDataList(data);
    };
  ...
  const filteredData = useMemo(() => {
    ...
    return dataList.filter((item) => item.tanggal === formatted);
  ...
  const formatTanggalIndo = (dateInput, withTime = false) => {
    ...
    const tanggal = `${hari[date.getDay()]}, ${String(date.getDate()).padStart(
      2,
      "0"
  ...
  return (
    <div className="w-full p-4 md:p-6">
      ...
      <div
        ref={calendarRef}
        className="mb-6 bg-white p-4 rounded-xl shadow relative"
      >
        ...
        <div className="flex flex-col md:flex-row gap-2">
          <input
            type="text"
            readOnly
            value={selectedDate ? formatTanggalIndo(selectedDate) : ""}
